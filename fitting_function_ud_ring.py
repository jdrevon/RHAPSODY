...
from rhapsody_init import model_rho_LM, model_rho_N, REG_method, max_iterations, tolerance
...
def UD_modeling(wavel_UD,wavel_TOT, q_TOT, V2_TOT, V2_ERR,\
    ...
    chi2= np.zeros(len(wavel_UD))
    chi2_red = np.zeros(len(wavel_UD))
    chi2_tot = np.zeros(len(wavel_UD))
    f_prior = np.zeros(len(wavel_UD))
    ...
    for i in range(len(wavel_UD)):
        ...
        if nb_UD != 1:
            ...
            f_prior[i] = f_prior_tmp
           
            # CHI2_TOT
            
            chi2_tot_tmp = chi2_tmp + f_prior_tmp
            chi2_tot[i]=chi2_tot_tmp
        ...
        if wavel_UD[i]<6:
            ...
            I_tot_LM[i_LM] = I_tot_norm
            i_LM = i_LM+1


        else :
            table_res_fit_N.add_row([wavel_UD[i]]+ np.round(res_F_mod,6).tolist())
            I_tot_N[i_N] = I_tot_norm
            i_N = i_N+1
    ...
    with open(PATH_OUTPUT_INT+'intensity_LM.dat', 'w') as f: f.write(str(data_intensity))
    ...
    with open(PATH_OUTPUT_INT+'intensity_N.dat', 'w') as f: f.write(str(data_intensity))


    # ALL LMFIT REPORT

    if nb_UD!=1 :        
        with open(PATH_OUTPUT_FIT_RES+ 'resultat_fitting_'+str(nb_UD)+'UD'+'.txt', "w+") as att_file:
            for ind in range(len(resultat)):
