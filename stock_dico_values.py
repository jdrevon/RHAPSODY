...
def stock_V2_from_dico_CONCATENATE(OIFITS_TOT):
    ...
    for k in range(len(OIFITS_TOT)):
        ...
        try:
            flag           = ma.concatenate(OIFITS_TOT[k]['VIS2']['FLAG'])
            q_tmp          = ma.concatenate(OIFITS_TOT[k]['VIS2']['BASELINE'].astype('float')/OIFITS_TOT[k]['VIS2']['WAVEL'].astype('float'))
            wavel_tmp      = ma.concatenate(OIFITS_TOT[k]['VIS2']['WAVEL'].astype('float'))
            U              = ma.concatenate(OIFITS_TOT[k]['VIS2']['U'].astype('float'))
            V              = ma.concatenate(OIFITS_TOT[k]['VIS2']['V'].astype('float'))
            V2_tmp         = ma.concatenate(OIFITS_TOT[k]['VIS2']['VIS2'].astype('float'))
            V2_err_tmp     = ma.concatenate(OIFITS_TOT[k]['VIS2']['VIS2_ERR'].astype('float'))

        except: # In the case only 1 wavelenght is present
            
            # print('INFO: ONLY 1 wavelenght is detected in the file')
            
            flag           = OIFITS_TOT[k]['VIS2']['FLAG']
            q_tmp          = OIFITS_TOT[k]['VIS2']['BASELINE'].astype('float')/OIFITS_TOT[k]['VIS2']['WAVEL'].astype('float')
            wavel_tmp      = OIFITS_TOT[k]['VIS2']['WAVEL'].astype('float')
            U              = OIFITS_TOT[k]['VIS2']['U'].astype('float')
            V              = OIFITS_TOT[k]['VIS2']['V'].astype('float')
            V2_tmp         = OIFITS_TOT[k]['VIS2']['VIS2'].astype('float')
            V2_err_tmp     = OIFITS_TOT[k]['VIS2']['VIS2_ERR'].astype('float')
